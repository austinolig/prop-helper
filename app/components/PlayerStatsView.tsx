...
export default function PlayerStatsView({ playerSeason }: PlayerStatsViewProps) {
  return (
    <div className="w-full max-w-6xl">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-foreground mb-2">
          {playerSeason.playerName} - {playerSeason.season} Season
        </h2>
        <p className="text-sm text-foreground/70">
          {playerSeason.team} • {playerSeason.games.length} Games
        </p>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse bg-white dark:bg-gray-900 rounded-lg shadow-sm">
          ...
          <tbody>
            {playerSeason.games.map((game, index) => (
